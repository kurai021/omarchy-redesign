...
<button
	type="button"
	onclick={copyCommand}
	class="group flex w-full items-center justify-between gap-2 min-w-0 rounded-md border border-border bg-background-storm/60 px-4 py-3 text-left transition-colors duration-fast hover:border-accent"
	aria-live="polite"
>
	<span
		class="flex min-w-0 items-center gap-2 font-mono text-xs sm:text-sm text-text"
	>
		<span class="text-accent select-none shrink-0">$</span>
		<span class="truncate min-w-0">{command}</span>
	</span>

	<span
		class="flex shrink-0 items-center gap-1.5 text-xs text-text-muted group-hover:text-accent ml-2"
	>
		{#if copied}
			<svg
